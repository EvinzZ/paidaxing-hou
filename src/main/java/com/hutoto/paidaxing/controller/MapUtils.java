  ...
  public static void main(String[] args) {
    try {
      ...
      for (String s : list) {
        ...
        stringBuilder.append("甘肃省").append("兰州市").append("七里河区").append(s);
        // 1、根据地址获取经纬度
        Map<String, String> lonAndLat = getLonAndLat(stringBuilder.toString(), "80a27c2512d0a4adc256d9bb1bf1945e");
//        System.out.println("转换后经纬度为：" + lonAndLat);
        System.out.println();
        System.out.println();
        System.out.println(s);
        System.out.println(lonAndLat.get("lng"));
        System.out.println(lonAndLat.get("lat"));
      }
      // 2、根据经纬度获取地址
    ...
    if (obj.get("status").toString().equals("1")) {
      JSONObject jobJSON =
          JSONObject.parseObject(
              obj.get("geocodes")
                  .toString()
                  .substring(1, obj.get("geocodes").toString().length() - 1));
